...
func BenchmarkParse(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		DefaultEnv.ParseFromJSONStr(scpt)
	}
}
...
`
	s, err := DefaultEnv.ParseFromJSONStr(scpt)
	if err != nil {
		panic(err)
	}
	b.ReportAllocs()

	tb := DefaultEnv.NewContext(map[string]any{
		"$": map[string]any{
			"name": "500",
			"age":  float64(44),
		},
...
`
	o, err := DefaultEnv.ParseFromJSONStr(scpt)
	if err != nil {
		panic(err)
	}
...
func TestAllFunc(t *testing.T) {
	...
	DefaultEnv.RegisterFunc("reterr", func(ctx *Context, args ...Val) any {
		return &Result{
			Err:  "return error",
			Data: "hello world",
		}
